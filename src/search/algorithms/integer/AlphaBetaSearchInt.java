...
public class AlphaBetaSearchInt extends MinMaxSearch implements AdversarialSearch<Integer> {
	...
	Pair<Node, Integer> alphaBetaMax( Node current,  Pair<Node, Integer> alpha,  Pair<Node, Integer> beta, int depthleft ) {
		...
		for ( Node node : current.adjacent() ) {
			...
			if( score >= beta.s ) {
				...
				//System.exit(-1);
				return new Pair<Node, Integer>(node, score);   // fail hard beta-cutoff
			}
			if( score > alpha.s ) {
	...
	Pair<Node, Integer> alphaBetaMin( Node current, Pair<Node, Integer> alpha, Pair<Node, Integer> beta, int depthleft ) {
		...
		for ( Node node : current.adjacent() ) {
			...
			if( score <= alpha.s ) {
				...
				resultState += 1;
				return new Pair<Node, Integer>(node, score); // fail hard alpha-cutoff
			}
			if( score < beta.s ) {
