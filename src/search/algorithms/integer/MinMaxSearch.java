package search.algorithms.integer;
...
public class MinMaxSearch implements AdversarialSearch<Integer> {
	...
	public MinMaxSearch(int depthLimit, long starttime)
	{
	...
	Pair<Node, Integer> max( Node current,  int depthleft ) {
		...
		return new Pair<Node, Integer>(bestNode, bestScore);
	}

	Pair<Node, Integer> min( Node current,  int depthleft ) {
		//System.out.println("max " +  current.getAction());
		nodecount++;   
		if ( depthleft == 0 || current.isLeaf() || timeIsUp() || interrupted) {
			resultState = 0; // reset result state;
